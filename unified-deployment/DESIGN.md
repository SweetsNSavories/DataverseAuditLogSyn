# Design Notes — Dataverse Audit Sync (unified-deployment)

This document is the deep-dive companion to the [TechCommunity blog post](../docs/blog/archiving-dataverse-audit-logs.md). The blog explains the *why*; this document explains the *how* in enough detail that you can adopt or fork the codebase confidently.

> **Reminder.** This is a reference implementation under MIT, **not a Microsoft product**. For most Dataverse-to-Azure replication needs the recommended starting points are [Azure Synapse Link for Dataverse](https://learn.microsoft.com/power-apps/maker/data-platform/azure-synapse-link-synapse) and [Microsoft Fabric Link for Dataverse](https://learn.microsoft.com/power-apps/maker/data-platform/azure-synapse-link-view-in-fabric). This pattern exists for the narrow case where you need the per-row `RetrieveAuditDetails` payload preserved before Dataverse prunes it.

- [The pattern in one diagram](#the-pattern-in-one-diagram)
- [Architecture in detail](#architecture-in-detail)
  - [The exact API call shape](#the-exact-api-call-shape)
  - [Concurrency is bounded *globally*, not per-entity](#concurrency-is-bounded-globally-not-per-entity)
  - [State container schema](#state-container-schema)
  - [Why `auditid` makes a perfect document key](#why-auditid-makes-a-perfect-document-key)
  - [How the half-open math works](#how-the-half-open-math-works)
  - [When the watermark refuses to advance](#when-the-watermark-refuses-to-advance)
- [A worked test run](#a-worked-test-run)
  - [1. Smoke test: one entity, two-hour window](#1-smoke-test-one-entity-two-hour-window)
  - [2. Partial-failure drill: prove the watermark holds](#2-partial-failure-drill-prove-the-watermark-holds)
  - [3. Realistic backlog: drain a multi-window range](#3-realistic-backlog-drain-a-multi-window-range)
  - [4. What to look for in a successful run](#4-what-to-look-for-in-a-successful-run)
- [How to adapt it](#how-to-adapt-it)
  - [Add or remove an entity](#add-or-remove-an-entity)
  - [Narrow which fields leave Dataverse](#narrow-which-fields-leave-dataverse)
  - [Add a new sink](#add-a-new-sink)
...
Three details do most of the resiliency work:

1. **Half-open windows (`ge` / `lt`)** — adjacent windows never overlap and never gap, no matter how many times you replay.
2. **The destination document key is the Dataverse `auditid` GUID** — re-writing the same audit twice is a no-op overwrite. Idempotency for free.
3. **The watermark moves *after* the write, not with it** — a typed `SinkPartialWriteError` ensures the loop replays the same window until it lands cleanly.
...
This is the single most important behavioural property of the pipeline. It is also a one-line decision — *raise vs return a count* — and in the wild it is often implemented the wrong way.

---

## A worked test run

The reference implementation ships with a small harness for validating the loop end-to-end before you point it at production.
...
The `SinkPartialWriteError` contract is the part of the pipeline most worth proving in a controlled setting. The repo includes `test_partial_failure.py`, a three-phase test that does the following with no live Dataverse traffic:
...
When this was exercised on a sandbox tenant covering the most recent 90 days, the run completed in single-digit minutes at a few hundred records per second — with the operational signal being not the absolute throughput but the *steady cadence of per-window log lines*. If those go ragged, look at lag, sink errors, or 429s before you scale up the in-flight concurrency.
...
A new sink is one new file plus one factory entry. The contract you implement is the `AuditSink` ABC in [`unified-deployment/sinks/base.py`](sinks/base.py):
...
Register it in [`unified-deployment/sinks/__init__.py`](sinks/__init__.py):
...
The fan-out trick worth knowing: setting the `ENTITY=account` env var causes the orchestrator to process *only* that entity. Spinning up N copies of the same image with N different `ENTITY` values gives you trivially parallel per-entity scaling without any orchestration changes, and each copy maintains its own watermark independently. Combined with a second application user, this is also how you scale past one user's service-protection budget.
...
- [`unified-deployment/main.py`](main.py) — the orchestrator (window loop, watermark logic, exception handling)
- [`unified-deployment/sinks/base.py`](sinks/base.py) — the `AuditSink` interface and the `SinkPartialWriteError` contract
- [`unified-deployment/sinks/cosmos_sink.py`](sinks/cosmos_sink.py) — the most-tested sink: HPK + serverless-aware container creation + idempotent upsert
- [`unified-deployment/test_partial_failure.py`](test_partial_failure.py) — the three-phase watermark-hold test described above
- [Top-level ARCHITECTURE.md](../ARCHITECTURE.md) — broader system context across the legacy and unified deployments
- [Top-level CONFIGURATION_REFERENCE.md](../CONFIGURATION_REFERENCE.md) — exhaustive `config.json` and env-var reference
